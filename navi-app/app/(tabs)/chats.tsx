...
export default function ChatsScreen() {
    ...
    const handleChatPress = (chat: Chat) => {
        if (chat.modelType === "Campus Navigator") {
            ...
            // 1. FIND THE SUBJECT NAME (e.g., "Programming") from the ID
            const subjectObj = (sampleData.subjects as { id: string; name: string }[]).find(
                (s) => s.id === chat.subject
            );
            const subjectName = subjectObj ? subjectObj.name : "General";

            // 2. NAVIGATE TO THE DYNAMIC SCREEN with the NAME
            router.push({
                pathname: `/assistants/ai-tutor/${subjectName}`,
                params: { chatId: chat.id }, // Optional: pass chat ID if you want to load history
            } as any);
        }
    };
